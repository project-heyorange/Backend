...
CREATE TABLE `USUARIO` (
  ...
  `NIVEL_EXPERIENCIA` varchar(100) DEFAULT NULL,
  PRIMARY KEY (`ID`),
  KEY `FK_MENTOR` (`ID_MENTOR`),
  KEY `FK_MENTORADO` (`ID_MENTORADO`),
  KEY `FK_DOMINIO_STATUS_USUARIO_USUARIO` (`ID_DOMINIO_STATUS_USUARIO`),
  KEY `FK_DOMINIO_TIPO_USUARIO_USUARIO` (`ID_DOMINIO_TIPO_USUARIO`),
  CONSTRAINT `FK_DOMINIO_STATUS_USUARIO_USUARIO` FOREIGN KEY (`ID_DOMINIO_STATUS_USUARIO`) REFERENCES `DOMINIO_STATUS_USUARIO` (`ID`),
  CONSTRAINT `FK_DOMINIO_TIPO_USUARIO_USUARIO` FOREIGN KEY (`ID_DOMINIO_TIPO_USUARIO`) REFERENCES `DOMINIO_TIPO_USUARIO` (`ID`),
  CONSTRAINT `FK_MENTOR` FOREIGN KEY (`ID_MENTOR`) REFERENCES `MENTOR` (`ID`),
  CONSTRAINT `FK_MENTORADO` FOREIGN KEY (`ID_MENTORADO`) REFERENCES `MENTORADO` (`ID`)
);
...
CREATE TABLE `HABILIDADE_USUARIO` (
  `ID` int NOT NULL AUTO_INCREMENT,
  `HABILIDADE_NOME` varchar(100) NOT NULL,
  `ID_USUARIO` int NOT NULL,
  PRIMARY KEY (`ID`),
  KEY `FK_MENTORADO_HABILIDADE_USUARIO` (`ID_USUARIO`),
  CONSTRAINT `FK_MENTORADO_HABILIDADE_USUARIO` FOREIGN KEY (`ID_USUARIO`) REFERENCES `USUARIO` (`ID`)
);

CREATE TABLE `NOVA_MENTORIA` (
  `ID` int NOT NULL AUTO_INCREMENT,
  `ID_MENTORADO` int NOT NULL,
  `ID_MENTOR` int NOT NULL,
  `DATA_MENTORIA` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`ID`),
  KEY `FK_MENTORADO_MENTORIA2` (`ID_MENTORADO`),
  KEY `FK_MENTOR_MENTORIA2` (`ID_MENTOR`)
);
